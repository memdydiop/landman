...
new #[Layout('layouts.front')] #[Title('Accueil — BTP, Aménagement, Lotissement')] class extends Component {
    ...
    {
        // Données en Cache avec TTL de 300 secondes
        ...
        // Normalisation stricte des fallbacks
        $testimonials = $testimonialsCached;
        if ($testimonials->isEmpty()) {
            $testimonials = collect([
                (object) ['name' => 'Kouassi Jean — Cocody', 'role' => 'Propriétaire Villa 4 pièces', 'content' => 'SIBEA-CI a livré notre villa dans les délais, avec un suivi VRD impeccable et un ACD sécurisé.', 'rating' => 5, 'avatar_path' => null],
                (object) ['name' => 'Awa Koné — Bingerville', 'role' => 'Gérante PME', 'content' => 'Lotissement Abatta : viabilisation et ACD obtenus sans accroc. SIBEA-CI maîtrise le foncier.', 'rating' => 5, 'avatar_path' => null],
                (object) ['name' => 'Yao Kouamé — Bouaké', 'role' => 'Conducteur de Travaux', 'content' => 'Collaboration BTP fluide, respect du budget et délais. Je recommande SIBEA-CI.', 'rating' => 5, 'avatar_path' => null]
            ]);
        }

        $partners = $partnersCached;
        if ($partners->isEmpty()) {
            $partners = collect(['NSIA Banque', 'SIB', 'BOA CI', 'SGCI', 'BACI'])->map(fn($n) => (object) [
                'name' => $n, 
                'logo_path' => null
            ]);
        }
...
<section class="bg-white">
        ...
        <div class="relative mx-auto flex w-full max-w-7xl items-center px-4 lg:px-8 py-10 sm:py-12">
            <div class="max-w-3xl border-l-4 border-white/30 pl-6">
                <div class="text-xs tracking-[0.3em] text-[#4d7aa3]">
                    {{ $hero['badge'] ?? 'SARL • 2022 • IDU CI-2022-0016466 Q' }}
                </div>
                <flux:heading level="1" class="mt-3 text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white! leading-[1.1] mb-6">
    ...
    <div class="bg-zinc-900 py-10 text-white">
        <div class="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 text-center lg:grid-cols-5 lg:px-8">
            <div>
                <div class="text-4xl font-black text-[#4d7aa3]">{{ number_format($stats['projects_completed'], 0, ',', ' ') }}</div>
                <div class="mt-1 text-xs tracking-widest text-zinc-400">PROJETS LIVRÉS</div>
            </div>
            <div>
                <div class="text-4xl font-black text-[#4d7aa3]">{{ number_format($stats['happy_clients'], 0, ',', ' ') }}</div>
                <div class="mt-1 text-xs tracking-widest text-zinc-400">CLIENTS SATISFAITS</div>
            </div>
            <div>
                <div class="text-4xl font-black text-[#4d7aa3]">{{ number_format($stats['workers'], 0, ',', ' ') }}</div>
            ...
            @forelse($recentWorks as $work)
                ...
                <a href="{{ route('front.projects.show', $work) }}"
                    ...
                    <div class="aspect-square overflow-hidden">
                        @if ($work->cover_path)
                            <img src="{{ Storage::disk('public')->url($work->cover_path) }}" alt="{{ $work->title }}" class="size-full object-cover transition group-hover:scale-110" loading="lazy" />
                        @else
                            <img src="https://images.unsplash.com/{{ $fallbacks[$loop->index % count($fallbacks)] }}?w=600&q=80&auto=format&fit=crop" alt="{{ $work->title }}" class="size-full object-cover" loading="lazy" />
                        @endif
                    </div>
                    <div class="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-zinc-900/80 to-transparent p-4 opacity-0 transition group-hover:opacity-100">
                        <div class="text-sm font-bold text-white">{{ \Illuminate\Support\Str::upper($work->title) }}</div>
                        <div class="text-xs text-zinc-300">{{ is_object($work->service_type) ? $work->service_type->label() : $work->service_type }}</div>
                    </div>
                    <div class="absolute right-3 top-3 rounded-full bg-white/90 p-1 opacity-0 group-hover:opacity-100">⧉</div>
                </a>
            @empty
                <div class="col-span-4 rounded-xl border border-dashed p-8 text-center text-zinc-500">Aucun projet publié.</div>
            @endforelse
        </div>
    </div>
            ...
            <div class="mt-8 overflow-x-auto rounded-2xl border border-zinc-200 bg-white">
                <table class="w-full min-w-[640px] text-sm">
                    <thead class="bg-zinc-900 text-white">
                        <tr>
                            <th class="px-4 py-3 text-left">Pôle</th>
                            <th class="px-4 py-3 text-left">Domaine</th>
                            <th class="px-4 py-3 text-left">Livrables</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($cmsServices as $svc)
                            ...
                            <tr class="border-t border-zinc-100">
                                ...
                                <td class="px-4 py-3">
                                    <a href="{{ route('front.services.show', $validKey) }}" class="text-xs font-bold tracking-widest text-primary hover:underline">DÉTAIL →</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    <div class="relative overflow-hidden bg-zinc-900 py-12">
        ...
        @else
            <img src="https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=1920&q=80&auto=format&fit=crop" alt="" class="absolute inset-0 size-full object-cover opacity-20" loading="lazy" />
        @endif
        <div class="relative mx-auto flex max-w-7xl flex-col items-center justify-between gap-6 px-4 lg:flex-row lg:px-8">
            <h2 class="text-2xl font-black text-white">{{ $banner['title'] }}</h2>
            <a href="{{ $banner['cta_url'] ?? route('front.contact') }}" class="rounded bg-primary px-8 py-3 text-sm font-bold text-white hover:bg-[#002244]">{{ $banner['cta_label'] ?? 'DEMANDER UN DEVIS' }}</a>
        </div>
    </div>
        ...
        <div class="mt-8 grid gap-6 md:grid-cols-3">
            @foreach ($testimonials as $t)
                <div class="rounded-xl border border-zinc-200 p-6 text-center">
                    @if (!empty($t->avatar_path) && Storage::disk('public')->exists($t->avatar_path))
                        <img src="{{ Storage::disk('public')->url($t->avatar_path) }}" alt="{{ $t->name }}" class="mx-auto size-16 rounded-full object-cover" loading="lazy" />
                    @else
                        <img src="https://i.pravatar.cc/100?img={{ $loop->index + 1 }}" alt="{{ $t->name }}" class="mx-auto size-16 rounded-full object-cover" loading="lazy" />
                    @endif
            ...
            @foreach ($team as $member)
                <div class="text-center">
                    @if(!empty($member['avatar']) && Storage::disk('public')->exists($member['avatar']))
                        <img src="{{ Storage::disk('public')->url($member['avatar']) }}" alt="{{ $member['name'] }}" class="mx-auto aspect-square w-full rounded-xl object-cover" loading="lazy" />
                    @else
                        <img src="https://i.pravatar.cc/400?img={{ $loop->index + 10 }}" alt="{{ $member['name'] }}" class="mx-auto aspect-square w-full rounded-xl object-cover" loading="lazy" />
                    @endif
    ...
    <div class="border-y border-zinc-200 bg-zinc-50 py-8">
        <div class="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-8 px-4 opacity-60 grayscale lg:px-8">
            @foreach ($partners as $partner)
                @if (!empty($partner->logo_path) && Storage::disk('public')->exists($partner->logo_path))
                    <img src="{{ Storage::disk('public')->url($partner->logo_path) }}" alt="{{ $partner->name }}" class="h-8" loading="lazy" />
                @else
                    <img src="https://via.placeholder.com/120x40/ffffff/000000?text={{ urlencode($partner->name) }}" alt="{{ $partner->name }}" class="h-8" loading="lazy" />
                @endif
            @endforeach
        </div>
    </div>
    ...
    <div class="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <div class="rounded-3xl bg-primary p-8 text-white lg:p-12">
            <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('front.contact') }}" class="rounded-full bg-white px-8 py-3 text-sm font-semibold text-[#002244] hover:bg-zinc-100">Demander un devis</a>
                    @php
                    ...
                    @endphp
                    <a href="https://wa.me/{{ $wa }}" target="_blank" class="rounded-full bg-emerald-600 p-3 text-white hover:bg-emerald-700 inline-flex items-center justify-center" aria-label="WhatsApp">
        ...
        <div class="mt-4 text-center">
            <a href="{{ route('front.home.africaspace') }}" class="text-xs tracking-widest text-zinc-500 hover:text-primary">Voir variante AfricaSpace — hero épuré laboratoire →</a>
        </div>
    </div>
</section>